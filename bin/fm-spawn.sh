...
launch_template() {
  ...
  codex)
    if [ "$kind" = secondmate ]; then
      printf '%s' 'codex __MODELFLAG____EFFORTFLAG__--sandbox workspace-write --ask-for-approval on-request "$(__OPINPUT__ encode launch-brief < __BRIEF__)"'
    else
      printf '%s' 'codex __MODELFLAG____EFFORTFLAG__--sandbox workspace-write --ask-for-approval on-request --disable hooks -c "notify=[\"bash\",\"-c\",\"touch __TURNEND__\"]" "$(__OPINPUT__ encode launch-brief < __BRIEF__)"'
    fi
    ;;
